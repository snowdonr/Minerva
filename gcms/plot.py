...
class GeneralPlotFrame(wx.Frame):
    ...
    def create_main_panel(self):
        ...
        self.canvas.mpl_connect('pick_event', self.on_pick)  # 'button_press_event'
        # self.canvas.mpl_connect('scroll_event', self.on_scroll_event)
        # self.canvas.mpl_connect('draw_event', self.on_draw_event)
        # self.weakReferenceOverride = self.on_pick  # Attempt to prevent garbage collection of callback
        ...
        self.vbox.Add(self.canvas, 1, wx.GROW)  # wx.LEFT | wx.TOP |
...
class SampleSetPlot(GeneralPlotFrame):
    ...
    def show_plot(self, aligned_data: list):
        ...
        # area_grid = aligned_data.T.applymap(lambda x: 0 if x is None else x.area)
        ...
        # rt_grid = aligned_data.T.applymap(lambda x: 0 if x is None else x.rt)
        # rt_grid[numpy.isclose(rt_grid, 0.0)] = numpy.nan
        ...
        self._last_data_filtered = numpy.array(aligned_data)[max_filter, :][rt_sorting, :]
        if USE_BOXPLOT_ONLY:
            plt = self.current_plot.boxplot(filtered_areas)
            # scatter_plt = self.current_plot.scatter(rt_filter.values[rt_sorting], mean_values, marker="+", c="blue", zorder=20, s=30, picker=5)
            ...
            average_x = numpy.mean(x_list, axis=0)
            average_y = numpy.mean(y_list, axis=0)
            if log_scale:
...
class PeakDetailPlots(GeneralPlotFrame):
    ...
    def create_main_panel(self):
        ...
        self.Layout()
        self.Show()
        # source_reference.formula
        # source_reference.synonyms
    ...
    def show_data(self, target_rt_index: int):
        ...
        self.update_right(max_sample_index, target_rt_index)
        self.update_plot()

    def update_right(self, sample_index, target_rt_index):
    ...
    def show_single_data(self, current_plot, source_reference):
        ...
        plt = current_plot.stem(masses, intensities, label=legend_str)
    ...
    def _rt_plot(self, current_plot, target_rt_index: int):
        ...
        peak_data = self._source_data[target_rt_index, self._select_sample_index]
        target_rt = peak_data.rt
        target_intensity = y_values[numpy.searchsorted(x_values, target_rt)]
        # sum(peak_data.ion_areas.values())  #
        target_rt_norm = peak_data.rt/self.TIME_NORM
    ...
    def _mass_spec_plot(self, current_plot: matplotlib.axes.SubplotBase, sample_index: int):
        ...
        stem_plot = current_plot.stem(peak_data.mass_spectrum.mass_list, peak_data.mass_spectrum.intensity_list)
    ...
    def _library_plot(self, current_plot, peak_data):
        current_plot.clear()
        ...
        library_ms = [x[1].mass_spec for x in found_reference_data]
        compare_scores = mass_spectrum.MassSpectrum.compare_to_target_ms(peak_data.mass_spectrum, library_ms)
        plot_list = []
        all_masses = []
        all_intensities = []
        marker_list = ["x", "+", "*", "v", "^", "o"]
        for index, (compound_name, reference) in enumerate(found_reference_data):
            ms = reference.mass_spec

            legend_str = f"{100*compare_scores[index]:.1f} {reference.name}({reference.formula}) mw:{reference.mw}"  # CAS {reference.cas}
            ...
            if numpy.any(numpy.isnan(intensities)):
                logging.info(f"NaN: Unknown mass spectrum for {compound_name}")
            all_masses.extend(ms.mass_list)
            all_intensities.extend(intensities)
            plot_list.append(plt)
        self._select_compound_data = found_reference_data
        string_list = ["All", ]
        string_list.extend([x[1].name if x[1] is not None else "?" for x in found_reference_data])
    ...
    def on_change_compound(self, event: wx.Event):
        ...
        if event.GetSelection() == 0:
            ...
            return
        _source, source_reference = self._select_compound_data[event.GetSelection()-1]
    ...
    def on_confirm_compound(self, _event: wx.Event):
        try:
            ...
            selected_reference = selected_raw_data[1]
            self._project.aligned_peaks[self._current_rt_index]
            
            self._select_sample_index
            
            self._select_compound_data
    ...
    def on_pick(self, event: wx.Event):
        ...
        self.update_right(self._select_sample_index, self._current_rt_index)
        self.update_plot()
        self._override_compound_update = False
...
class LoadedDataPlot(GeneralPlotFrame):
    '''
    Plots raw GCMS data for verification and testing
    '''
    def __init__(self, mass_spec_frame, sample_set=None):
        super().__init__(title='GCMS Debug plot')
