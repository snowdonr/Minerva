...
class MainUI(wx.Frame):
    '''
    UI related functions, resulting data is stored in the project
    '''
    ...
    def update_directory_path(self, _event, text_ctrl: wx.TextCtrl):
        ...
        if file_menu.ShowModal() == wx.ID_OK:
            ...
            if path_value.is_dir():
                ...
                print(f"Could not find {file_menu.Path}")
    ...
    def _finished_loading(self, delayed_result):
        self._status_loading = False
        self._load_button.Enable(enable=True)
...
#         test_sample_data = self._project.sample_set[0]
#         peaks = test_sample_data.get_filtered_peaks(lambda x: x.area>30000)
#         self._project.identify_peak_plot(test_sample_data, peaks[103])
        return
